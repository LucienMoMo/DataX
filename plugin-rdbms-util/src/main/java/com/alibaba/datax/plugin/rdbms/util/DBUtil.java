...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
    ...
    private static synchronized Connection connect(DataBaseType dataBaseType,
        ...
        try {
            // 获取ddconfig中的用户配置信息
            prop.setProperty("user", DDConfig.getSingleton().getUserInfoSetting(prop.getProperty("user")));
            prop.setProperty("password", DDConfig.getSingleton().getUserInfoSetting(prop.getProperty("password")));
            // 加载驱动类
